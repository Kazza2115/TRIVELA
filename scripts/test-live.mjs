// TEST one-off : simule un match EN DIRECT pendant ~2 min puis nettoie tout.
...
// Scénario sur ~2 min : 0:0 → 1:0 (34') → 1:1 (58') → 2:1 (72')
function frame(t) {                       // t = secondes écoulées
  if (t < 25)  return { status: '1H', elapsed: 12, h: 0, a: 0, scorers: [] }
  if (t < 60)  return { status: '1H', elapsed: 34, h: 1, a: 0, scorers: [
    { p: 'R. Jiménez', s: 'home', t: 34, og: false, pen: false } ] }
  if (t < 95)  return { status: '2H', elapsed: 58, h: 1, a: 1, scorers: [
    { p: 'R. Jiménez', s: 'home', t: 34, og: false, pen: false },
...
async function main() {
  const start = Date.now(), DURATION = 125_000   // ~2 min
  let lastScore = ''
  console.log(`▶️  TEST live démarré sur ${MATCH_ID} (~2 min)`)
